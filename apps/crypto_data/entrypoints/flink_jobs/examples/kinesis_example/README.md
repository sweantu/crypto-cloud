...
When running on Amazon Managed Service for Apache Flink the runtime configuration comes from _Runtime Properties_.
...
4. Set the environment variable `RUN_LOCAL=true`. You can do from the prompt or in the run profile of the IDE
5. Run `main.py`
...
If you forget the set the environment variable `RUN_LOCAL=true` or forget to run `mvn package` the application fails on start.
...
> parameters, the Amazon Managed Service for Apache Flink may report as _Running_ but the job fails to start.
...
4. In the Managed Flink application console, enter _Configure_, scroll down and press _Save Changes_
   ...
   - If the application was not running (in Ready state) you need to click _Run_ to restart it with the new code
...
> console or using the [_UpdateApplication_](https://docs.aws.amazon.com/managed-flink/latest/apiv2/API_UpdateApplication.html)
> API.
...
> Note: because we are using Maven to build a "fat-jar", we should not use the "SQL" dependencies, named "_-sql-_".
