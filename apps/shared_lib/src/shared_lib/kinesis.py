...
class KinesisClient:
            ...
            while True:
                for shard_id, iterator in list(shard_iters.items()):
                    try:
                        ...
                        logger.info(f"❌ Error fetching records from {shard_id}: {e}")
                        continue

                    records = resp.get("Records", [])
                    if records:
                        total += len(records)
                        logger.info(f"🔢 Total records consumed so far: {total}")
    ...
    def get_shard_iters(self, iterator_type, stream_name):
        ...
        logger.info(f"📦 Found {len(shards)} shard(s) in {stream_name}")
