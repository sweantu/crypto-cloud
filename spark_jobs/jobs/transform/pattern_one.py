...
if table_exists(spark, serving_db, pattern_one_table):
    sql_stmt = f"""
    select ema7, ema20 from {serving_db}.{pattern_one_table}
...
if table_exists(spark, serving_db, pattern_one_table):
    df.writeTo(f"{serving_db}.{pattern_one_table}").overwritePartitions()
else:
    df.writeTo(f"{serving_db}.{pattern_one_table}").tableProperty(
        "format-version", "2"
